...
class EnhancedSeasonalColorDatabase:
    ...
    def extract_enhanced_features(self, image_path):
        ...
        try:
            ...
            hair_features = self._extract_hair_features_improved(
                image, image_cv, image_lab, image_hsv, face_locations[0], landmarks)
    ...
    def _extract_hair_features_improved(self, image_rgb, image_bgr, image_lab, image_hsv, face_location, landmarks):
        """Improved hair feature extraction based on the second code snippet"""
        features = {}
        ...
        # Define hair region - above face and slightly wider
        hair_top = max(0, top - int(0.7 * (bottom - top)))
        hair_bottom = top + int(0.2 * (bottom - top))  # Include some forehead
        hair_left = max(0, left - int(0.2 * (right - left)))
        hair_right = min(image_rgb.shape[1], right + int(0.2 * (right - left)))
        
        # Fill the hair region in the mask
        mask[hair_top:hair_bottom, hair_left:hair_right] = 255
        
        # Exclude face area from the mask
        for facial_feature in ['left_eyebrow', 'right_eyebrow', 'nose_tip', 'nose_bridge', 'left_eye', 'right_eye']:
            if facial_feature in landmarks:
                points = np.array(landmarks[facial_feature], dtype=np.int32)
                cv2.fillPoly(mask, [points], 0)
        
        # Get masked regions for all color spaces
        hair_region_bgr = cv2.bitwise_and(image_bgr, image_bgr, mask=mask)
        hair_region_lab = cv2.bitwise_and(image_lab, image_lab, mask=mask)
        hair_region_hsv = cv2.bitwise_and(image_hsv, image_hsv, mask=mask)
        
        # Get hair pixels (excluding masked areas)
        hair_pixels_bgr = hair_region_bgr[np.where(mask == 255)]
        hair_pixels_lab = hair_region_lab[np.where(mask == 255)]
        hair_pixels_hsv = hair_region_hsv[np.where(mask == 255)]
        
        if len(hair_pixels_bgr) == 0:
            return features  # Return empty features if no hair detected
        
        # Calculate mean and std for each color space
        # BGR
        mean_bgr = np.mean(hair_pixels_bgr, axis=0)
        std_bgr = np.std(hair_pixels_bgr, axis=0)
        features.update({
            'hair_b': mean_bgr[0],
            'hair_g': mean_bgr[1],
            'hair_r': mean_bgr[2],
            'hair_b_std': std_bgr[0],
            'hair_g_std': std_bgr[1],
            'hair_r_std': std_bgr[2],
        })
        
        # LAB
        mean_lab = np.mean(hair_pixels_lab, axis=0)
        std_lab = np.std(hair_pixels_lab, axis=0)
        features.update({
            'hair_l': mean_lab[0],
            'hair_a': mean_lab[1],
            'hair_b_lab': mean_lab[2],
            'hair_l_std': std_lab[0],
            'hair_a_std': std_lab[1],
            'hair_b_lab_std': std_lab[2],
        })
        
        # HSV
        mean_hsv = np.mean(hair_pixels_hsv, axis=0)
        std_hsv = np.std(hair_pixels_hsv, axis=0)
        features.update({
            'hair_h': mean_hsv[0],
            'hair_s': mean_hsv[1],
            'hair_v': mean_hsv[2],
            'hair_h_std': std_hsv[0],
            'hair_s_std': std_hsv[1],
            'hair_v_std': std_hsv[2],
        })
        
        # Additional hair features
        h, s, v = mean_hsv
        features['hair_warmth'] = 1.0 if (h < 30 or h > 330) else 0.0  # Reddish tones
        features['hair_coolness'] = 1.0 if (120 < h < 240) else 0.0  # Bluish tones
        features['hair_lightness'] = v / 255.0  # Normalized value
...
class ImprovedSeasonalColorModel:
    ...
    def preprocess_data(self):
        ...
        self.df['dark_season_indicator'] = (
            ...
            -0.7 * self.df['hair_l'] +   # Lower L values indicate darker hair
    ...
    def train_model(self, test_size=0.2, random_state=42):
        ...
        param_dist = {
            ...
            'classifier__max_depth': [None, 20, 30], # Tree complexity, max depth of trees
            'classifier__min_samples_split': [2, 5, 5], # Minimum samples required to split a node.
            'classifier__min_samples_leaf': [1, 2], # Minimum samples required at a leaf node.
            'classifier__max_features': ['sqrt', 'log2', None], #Number of features considered for splitting at each node.
        ...
        # Save model
        joblib.dump(self.model, 'colour_model.joblib')
...
def main():
    ...
    df.to_csv('seasondatabase.csv', index=False)
    print("Database created and saved successfully!")

    # Train the model
    print("\nTraining the model...")
    model = ImprovedSeasonalColorModel('seasondatabase.csv')
    print(f"Looking for CSV at: {os.path.abspath('seasondatabase.csv')}")
    model.train_model()
