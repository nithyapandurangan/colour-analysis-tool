...
class EnhancedSeasonalColorDatabase:
    def __init__(self, image_directory):
        ...
        self.hair_interpreter = tf.lite.Interpreter(model_path="hair_segmenter.tflite")
        self.hair_interpreter.allocate_tensors()
        self.hair_input_details = self.hair_interpreter.get_input_details()
        
    def _segment_hair(self, image_cv):
        """Segment hair using the TFLite model and return hair mask"""
        # Resize and convert to RGB
        rgb_image = cv2.cvtColor(cv2.resize(image_cv, (512, 512)), cv2.COLOR_BGR2RGB)
        
        # Add dummy alpha channel (4th dimension)
        input_image = np.concatenate([
            rgb_image, 
            np.ones((512, 512, 1), dtype=np.float32)  # Alpha=1 (opaque)
        ], axis=-1)
        ...
        # Run inference
        self.hair_interpreter.set_tensor(self.hair_input_details[0]['index'], input_image)
        self.hair_interpreter.invoke()
        
        # Get segmentation mask
        mask = self.hair_interpreter.get_tensor(self.hair_interpreter.get_output_details()[0]['index'])[0]
        hair_mask = np.argmax(mask, axis=-1)
        hair_mask = cv2.resize(hair_mask.astype(np.uint8), 
    ...
    def extract_enhanced_features(self, image_path):
        try:
            ...
            # Extract hair features using segmentation model
            hair_features = self._extract_hair_features_with_segmentation(
    ...
    def _extract_hair_features_with_segmentation(self, image_cv, image_lab, image_hsv):
        """Extract hair features using hair segmentation model"""
        features = {}
        
        # Get hair mask
        hair_mask = self._segment_hair(image_cv)
        
        if hair_mask is None or np.sum(hair_mask) == 0:
            # If no hair detected, return default values
            for i in range(3):  # Maintain same feature structure as original
                for color_space in ['b', 'g', 'r', 'l', 'a', 'b_lab', 'h', 's', 'v']:
                    features[f'hair_region{i}_{color_space}'] = 0
            return features
        
        # Extract hair pixels from each color space
        hair_pixels_bgr = image_cv[hair_mask == 1]
        hair_pixels_lab = image_lab[hair_mask == 1]
        hair_pixels_hsv = image_hsv[hair_mask == 1]
        
        # Sample 3 regions (to maintain same feature structure as original)
        for i in range(3):
            # For each region, take a random sample of hair pixels
            if len(hair_pixels_bgr) > 0:
                sample_size = min(100, len(hair_pixels_bgr))
                sample_indices = np.random.choice(len(hair_pixels_bgr), sample_size, replace=False)
                
                # BGR values
                bgr_sample = hair_pixels_bgr[sample_indices]
                ...
                # LAB values
                lab_sample = hair_pixels_lab[sample_indices]
                lab_mean = np.mean(lab_sample, axis=0)
                features.update({
                ...
                # HSV values
                hsv_sample = hair_pixels_hsv[sample_indices]
                hsv_mean = np.mean(hsv_sample, axis=0)
                features.update({
                ...
                # If no hair pixels in this sample, use zeros
                for color_space in ['b', 'g', 'r', 'l', 'a', 'b_lab', 'h', 's', 'v']:
                    features[f'hair_region{i}_{color_space}'] = 0
        
        # Additional hair features based on entire hair region
        if len(hair_pixels_bgr) > 0:
            # Dominant color via K-means
            kmeans = KMeans(n_clusters=3, n_init=10).fit(hair_pixels_bgr)
            dominant_bgr = kmeans.cluster_centers_[np.argmax(np.bincount(kmeans.labels_))]
            
            # Add dominant color features
            features.update({
                'hair_dominant_b': dominant_bgr[0],
                'hair_dominant_g': dominant_bgr[1],
                'hair_dominant_r': dominant_bgr[2],
            })
            
            # Add hair coverage feature
            hair_coverage = np.sum(hair_mask) / (hair_mask.shape[0] * hair_mask.shape[1])
            features['hair_coverage'] = hair_coverage
            
            # Add hair color variance features
            for j, channel in enumerate(['b', 'g', 'r']):
                features[f'hair_{channel}_variance'] = np.std(hair_pixels_bgr[:, j])
    ...
    def _extract_contrast_features(self, face, face_lab, face_hsv):
        ...
        # Enhanced features for autumn/winter
